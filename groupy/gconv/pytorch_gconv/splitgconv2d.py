...
class SplitGConv2D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, bias=True): #, input_stabilizer_size=1, output_stabilizer_size=4): , dtype=np.float32):
        ...
        self.inds = self.make_transformation_indices(ksize=self.ksize)
    ...
    def make_transformation_indices(self, ksize):
        ...
        # this should better be implemented in subclasses, thus uncomment
    ...
    def forward(self, input):
        ...
        input = input.view(input_shape[0], self.in_channels*self.input_stabilizer_size, input_shape[-2], input_shape[-1])  # this only has an effect if the shape isn' like this anyway
