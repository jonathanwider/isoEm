...
def central_index(sz):
    """Helper to calculate the central index for even and odd length"""
    return int((sz - sz % 2) / 2.)


def hexa2cartesian(n1, n2):
    """
    Convert from "standard" hexa lattice to cartesian grid
    """
    x = n1 + 0.5 * n2
    y = (0.5 * np.sqrt(3)) * n2
    return x, y
...
def zigzaghexa_mask(radius):
    """
    Create a mask of given size, convert to zigzag
    """
    n1, n2 = zigzaghexa2hexa(*centered_meshgrid(2 * radius + 1, 2 * radius + 1))
    # Manhattan distance from the origin in zigzaghexa lattice
