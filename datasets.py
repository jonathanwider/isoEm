    ...
    if description["TIMESCALE"] == "MONTHLY":
        if description["GRID_TYPE"] == "Ico":
            raise NotImplementedError(
                "Monthly timescale was only investigated on Flat grid.")
    ...
    for dst in description["DATASETS_USED"]:
        if description["GRID_TYPE"] == "Flat":
            if description["TIMESCALE"] == "YEARLY":
                d_path = os.path.join(
                    dataset_folder, description["CLIMATE_MODEL"], "Original", "{}_yearly.nc".format(dst))
                datasets[dst] = netCDF4.Dataset(d_path, "a")
            elif description["TIMESCALE"] == "MONTHLY":
                d_path = os.path.join(
                    dataset_folder, description["CLIMATE_MODEL"], "Original", "{}.nc".format(dst))
                datasets[dst] = netCDF4.Dataset(d_path, "a")
            else:
                raise NotImplementedError(
                    "Only MONTHLY and YEARLY timescales implemented.")
            ...
            raise NotImplementedError(
                "Only Ico and Flat grids were implemented.")
    return datasets
...
def get_shared_timesteps(description, dataset_folder):
    ...
    try:
        ...
        units = tuple(
            [dset.variables["t"].units for dset in datasets.values()])
        cals = tuple(
            [dset.variables["t"].calendar for dset in datasets.values()])
    except KeyError:
        ts = tuple(
            [dset.variables["time"][:].data for dset in datasets.values()])
        units = tuple(
            [dset.variables["time"].units for dset in datasets.values()])
        cals = tuple(
            [dset.variables["time"].calendar for dset in datasets.values()])

    if description["TIMESCALE"] == "YEARLY":
        ys = [set(util.get_years_months(t, units[i], cals[i])[0])
              for (i, t) in enumerate(ts)]
        common_dates = set.intersection(*ys)
    elif description["TIMESCALE"] == "MONTHLY":
        years_l = [util.get_years_months(t, units[i], cals[i])[
            0] for (i, t) in enumerate(ts)]
        months_l = [util.get_years_months(t, units[i], cals[i])[
...
def load_variables_and_timesteps(description, dataset_folder):
    ...
    if description["GRID_TYPE"] == "Flat":
        description["T_UNITS"], description["CALENDAR"] = util.load_units_cals(
            description, list(datasets.values()))
    elif description["GRID_TYPE"] == "Ico":
        ds_6nb = [ds["6_nb"] for ds in list(datasets.values())]
        description["T_UNITS"], description["CALENDAR"] = util.load_units_cals(
            description, list(ds_6nb))
    else:
    ...
    for dataset_name, dataset in datasets.items():  # loop over all used datasets
        ...
        for variable_name in dict(description["PREDICTOR_VARIABLES"], **description["TARGET_VARIABLES"])[dataset_name]:
            if description["GRID_TYPE"] == "Flat":
                description["LATITUDES"], description["LONGITUDES"] = util.load_longitudes_latitudes(
                    description, dataset)

                # only if time dimension is not trivial
                if dataset.variables[variable_name][:].data.shape[0] > 1:
                ...
                try:
                    ...
                    variables[dataset_name][variable_name].data[variables[dataset_name]
                                                                [variable_name].data == missing_value] = np.nan
                ...
                if dataset_name in description["TARGET_VARIABLES"].keys():
                    masks[dataset_name][variable_name] = np.ma.getmaskarray(
                        variables[dataset_name][variable_name])

            elif description["GRID_TYPE"] == "Ico":
                variables[dataset_name][variable_name] = {}
                # loop over subfiles: containing points with 5 and 6 nbs
                for subdataset_name, subdataset in dataset.items():
                    # only if time dimenion is not trivial
                    if subdataset.variables[variable_name][:].shape[0] > 1:
                    ...
                    try:
                        ...
                        variables[dataset_name][variable_name][subdataset_name].data[variables[dataset_name]
                                                                                     [variable_name][subdataset_name].data == missing_value] = np.nan
                    except AttributeError:
                        pass
                variables[dataset_name][variable_name] = combine_variables(
                    description, variables[dataset_name][variable_name])
            else:
                raise NotImplementedError(
                    "Only Ico and Flat grids implemented")
    res_masks = {}
    res_variables = {}

    for dataset_name, dataset in variables.items():  # loop over all used datasets
        v = dict(description["PREDICTOR_VARIABLES"],
                 **description["TARGET_VARIABLES"])
        # loop over all variables we want to use from this dataset
        for variable_name in v[dataset_name]:
    ...
    masked_timesteps = np.zeros(list(res_variables.values())[
                                0].shape[0], dtype=bool)
    for vs in description["PREDICTOR_VARIABLES"].values():
        for v in vs:
            if description["GRID_TYPE"] == "Flat":
                if (np.ma.getmaskarray(res_variables[v]) != False).any():
                    m = np.where(np.mean(np.ma.getmaskarray(
                        res_variables[v]), axis=(-1, -2, -3)) > 0)[0]
                    masked_timesteps[m] = True
            elif description["GRID_TYPE"] == "Ico":
                m = np.where(np.mean(np.ma.getmaskarray(
                    res_variables[v]), axis=(-1, -2)) > 0)[0]
                masked_timesteps[m] = True
            else:
                raise NotImplementedError(
                    "Only Ico and Flat grids implemented")
...
def combine_variables(description, dataset_dict):
    ...
    combined_data = np.ma.zeros(
        dataset_dict["6_nb"].shape[:-1] + (dataset_dict["6_nb"].shape[-1] + 10,))
...
def create_yearly_dataset(description, dataset_folder, output_folder):
    ...
    assert np.logical_xor("TEST_FRACTION" in description.keys(),
                          "SPLIT_YEAR" in description.keys())
    assert description["TIMESCALE"] == "YEARLY"

    print("loading variables")
    # load the selected climate variables.
    variables, masks, c_dates = load_variables_and_timesteps(
        description, dataset_folder)
    ...
    predictors = np.concatenate(
        tuple([variables[p_var] for p_var in pvars]), axis=1)
    targets = np.concatenate(
        tuple([variables[t_var] for t_var in tvars]), axis=1)
    ...
    if "INDICES_TEST" in description.keys():
        x_train, x_test = util.train_test_split_by_indices(
            description, predictors)
        y_train, y_test = util.train_test_split_by_indices(
            description, targets)
        indices_train, indices_test = util.train_test_split_by_indices(
            description, indices)
        timesteps_train, timesteps_test = util.train_test_split_by_indices(
            description, c_dates)
        if description["GRID_TYPE"] == "Flat":
            masks_train, masks_test = util.train_test_split_by_indices(
                description, masks)
    elif "TEST_FRACTION" in description.keys():
        seed = np.random.randint(
            np.iinfo(np.uint32).min, np.iinfo(np.uint32).max, dtype=np.uint32)
        x_train, x_test = util.train_test_split_by_proportion(
            description, predictors, seed)
        y_train, y_test = util.train_test_split_by_proportion(
            description, targets, seed)
        indices_train, indices_test = util.train_test_split_by_proportion(
            description, indices, seed)
        timesteps_train, timesteps_test = util.train_test_split_by_proportion(
            description, c_dates, seed)
        if description["GRID_TYPE"] == "Flat":
            masks_train, masks_test = util.train_test_split_by_proportion(
                description, masks, seed)
    else:
    ...
    if description["GRID_TYPE"] == "Flat":
        x_train = x_train.reshape(
            x_train.shape[0], len(pvars), -1, x_train.shape[-1])
        x_test = x_test.reshape(
            x_test.shape[0], len(pvars), -1, x_test.shape[-1])
        y_train = y_train.reshape(
            y_train.shape[0], len(tvars), -1, y_train.shape[-1])
        y_test = y_test.reshape(
            y_test.shape[0], len(tvars), -1, y_test.shape[-1])
        masks_train = masks_train.reshape(
            masks_train.shape[0], len(tvars), -1, masks_train.shape[-1])
        masks_test = masks_test.reshape(
            masks_test.shape[0], len(tvars), -1, masks_test.shape[-1])
    elif description["GRID_TYPE"] == "Ico":
        ico = icosahedron.Icosahedron(r=description["RESOLUTION"])
        charts = ico.get_charts_cut()
        x_train = x_train.reshape(
            x_train.shape[0], len(pvars), -1, charts.shape[-2])
        x_test = x_test.reshape(
            x_test.shape[0], len(pvars), -1, charts.shape[-2])
        y_train = y_train.reshape(
            y_train.shape[0], len(tvars), -1, charts.shape[-2])
        y_test = y_test.reshape(
            y_test.shape[0], len(tvars), -1, charts.shape[-2])
    else:
...
def load_variables_and_timesteps_months(description, dataset_folder):
    ...
    # make sure that all datasets that have a non-trivial time axis share the same calendar and units.
    description["CALENDAR"], description["T_UNITS"] = util.load_units_cals(
        description, list(datasets.values()))
    ...
    for dataset_name, dataset in datasets.items():  # loop over all used datasets
        ...
        for variable_name in dict(description["PREDICTOR_VARIABLES"], **description["TARGET_VARIABLES"])[dataset_name]:
            ...
            if dataset.variables[variable_name][:].data.shape[0] > 1:
                ...
                indices_selected_months = [np.squeeze(
                    months_indices[i]) for i in description["MONTHS_USED"]]
                # we need to sort because otherwise dataset is not ordered chronologically but one sorted by months.
                indices_selected_months = np.sort(
                    np.concatenate(tuple(indices_selected_months)))
            ...
            try:
                ...
                for d_m in description["MONTHS_USED_IN_PREDICTION"]:
                    ...
                    variables[dataset_name][variable_name][d_m].data[(
                        variables[dataset_name][variable_name][d_m].data == missing_value)] = np.nan
            except AttributeError:
                for d_m in description["MONTHS_USED_IN_PREDICTION"]:
                    variables[dataset_name][variable_name][d_m] = data[sel_i + d_m, ...]
        ...
        for v in vs:
            for dm in description["MONTHS_USED_IN_PREDICTION"]:
                if (np.ma.getmaskarray(res_variables[v][dm]) != False).any():
                    m = np.where(np.mean(np.ma.getmaskarray(
                        res_variables[v][dm]), axis=(-1, -2)) > 0)[0]
                    masked_timesteps[m] = True
    ...
    for key in res_masks.keys():
        assert(key not in util.flatten(
            description["PREDICTOR_VARIABLES"].values()))
...
def load_variables_and_timesteps_precip_weighted(description, dataset_folder):
    ...
    ) or ["precip"] in description["PREDICTOR_VARIABLES"].values()
    assert description["TIMESCALE"] == "YEARLY"
    assert description["GRID_TYPE"] == "Flat"
    variables = {}
    masks = {}
    ...
    description["CALENDAR"], description["T_UNITS"] = util.load_units_cals(
        description, list(datasets_yearly.values())+list(datasets_monthly.values()))
            ...
            if dataset.variables[variable_name][:].data.shape[0] > 1:
                ...
                if variable_name == "prec" or variable_name == "precip":
                    for i, yr in enumerate(c_dates):
                        i_mask = (yr == years)
                        var_yearly_data[i, ...] = np.ma.average(
                            masked_var[i_mask, ...], axis=0)
                else:
                    for i, yr in enumerate(c_dates):
                        i_mask = (yr == years)
...
def create_precip_weighted_dataset(description, dataset_folder, output_folder):
    ...
    print("loading variables")
    # load the selected climate variables.
    variables, masks, c_dates = load_variables_and_timesteps_precip_weighted(
        description, dataset_folder)
...
def create_monthly_dataset(description, dataset_folder, output_folder):
    ...
    print("loading variables")
    # load the selected climate variables.
    variables, masks, c_dates = load_variables_and_timesteps_months(
        description, dataset_folder)
    # split the variables into predictors and targets.
    pvars = util.flatten(description["PREDICTOR_VARIABLES"].values())
    tvars = util.flatten(description["TARGET_VARIABLES"].values())

    predictors = [[variables[p_var][d_m]
                   for d_m in description["MONTHS_USED_IN_PREDICTION"]] for p_var in pvars]
    predictors = np.array(predictors)
    predictors = predictors.reshape(
        predictors.shape[0]*predictors.shape[1], *predictors.shape[2:])
    ...
    x_train = x_train.reshape(x_train.shape[0], len(
        description["MONTHS_USED_IN_PREDICTION"])*len(pvars), -1, x_train.shape[-1])
    x_test = x_test.reshape(x_test.shape[0], len(
        description["MONTHS_USED_IN_PREDICTION"])*len(pvars), -1, x_test.shape[-1])
    y_train = y_train.reshape(
        y_train.shape[0], len(tvars), -1, y_train.shape[-1])
    y_test = y_test.reshape(y_test.shape[0], len(tvars), -1, y_test.shape[-1])
    masks_train = masks_train.reshape(
